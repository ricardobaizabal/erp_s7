...
Partial Public Class ComplementoDePagos40
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ObjCat.Catalogo(cmbCliente, "EXEC pMisClientes @cmd=1 ,@estatusid=1", 0)
    ...
    Private Sub btnCreateInvoice_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCreateInvoice.Click
        If Page.IsValid Then
            ...
            m_xmlDOM.Save(Server.MapPath("~/portalcfd/cfd_storage/") & "s7_" & serie.Value & folio.Value & ".xml")
            '
            '   Realiza Timbrado
            '
            If folio.Value > 0 Then
                Try
                    ...
                    '   Obtiene el UUID
                    '
                    Dim filePath As String = Server.MapPath("~/portalcfd/cfd_storage/") & "s7_" & serie.Value & folio.Value & "_timbrado.xml"
                    Dim UUID() As String
                    ReDim UUID(0)
                    '
                    ...
                    '   Genera PDF
                    '
                    If Not File.Exists(Server.MapPath("~/portalcfd/pdf") & "\" & "s7_" & serie.Value & folio.Value & ".pdf") Then
                        GuardaPDF(GeneraPDF(Session("CFD")), Server.MapPath("~/portalcfd/pdf") & "\" & "s7_" & serie.Value & folio.Value & ".pdf")
                    End If
                    '
                ...
                '        File.WriteAllBytes(Server.MapPath("~/portalcfd/cfd_storage") & "\" & "s7_" & serie.Value & folio.Value & "_timbrado.xml", puerto.XML)
    ...
    Private Function Comprimir()
        ...
        zip.AddFile(Server.MapPath("~/portalcfd/cfd_storage/") & "s7_" & serie.Value.ToString & folio.Value.ToString & ".xml", "")
    ...
    Private Function Descomprimir(ByVal data5 As Byte())
        ...
        If File.Exists(Path & archivo) Then
            System.IO.File.Copy(Path & archivo, Path & "s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml")
        End If
    ...
    Private Sub CrearNodoPagos(ByVal Nodo As XmlNode)
        ...
        Dim TotalBaseDr As Decimal = 0
        Dim TotalBasep As Decimal = 0

        Dim TotalImporteDR As Decimal = 0
        Dim TotalImporteP As Decimal = 0
        ...
        Try
            ...
            While rs.Read
                ...
                Dim ObjData As New DataControl
                ds = ObjData.FillDataSet("exec pCFDTraslados @cmd=7, @cfdid='" & rs("cfdid").ToString & "', @pagoid='" & Session("PAGOCFD").ToString & "'")
                ObjData = Nothing

                If ds.Tables(0).Rows.Count > 0 Then
                    For Each row As DataRow In ds.Tables(0).Rows
                        TotalTrasladosImpuestoIVA16 = TotalTrasladosImpuestoIVA16 + row("iva")
                        TotalTrasladosBaseIVA16 = TotalTrasladosBaseIVA16 + row("siniva")

                        TotalBaseDr = TotalBaseDr + row("siniva")
                        TotalBasep = TotalBasep + row("iva")

                        TotalImporteDR = TotalImporteDR + row("iva")
                        TotalImporteP = TotalImporteP + row("iva")

                        TrasladoDR.SetAttribute("ImpuestoDR", row("impuesto"))
                        TrasladoDR.SetAttribute("TipoFactorDR", row("tipofactor"))
                        TrasladoDR.SetAttribute("TasaOCuotaDR", row("tasaOcuota"))

                        TrasladoP.SetAttribute("ImpuestoP", row("impuesto"))
                        TrasladoP.SetAttribute("TipoFactorP", row("tipofactor"))
                        TrasladoP.SetAttribute("TasaOCuotaP", row("tasaOcuota"))

                        'TrasladoP.SetAttribute("ImporteP", Format(row("importe"), "#0.00"))
                    Next

                End If

                TrasladoDR.SetAttribute("ImporteDR", Format((TotalImporteDR), "#0.000000"))
                TrasladoDR.SetAttribute("BaseDR", Format((TotalBaseDr), "#0.000000"))
                ...
                IndentarNodo(Pago, 3)
            End While
            TrasladoP.SetAttribute("ImporteP", Format((TotalImporteP), "#0.000000"))
            TrasladoP.SetAttribute("BaseP", Format((TotalTrasladosBaseIVA16), "#0.000000"))
        ...
        If cmbMoneda.SelectedValue = 1 Then
            MontoTotalPagos = total
        Else
            MontoTotalPagos = total * tipocambio
        End If

        Totales = CrearNodo("pago20:Totales")
        If TotalTrasladosImpuestoIVA16 > 0 Then
            Totales.SetAttribute("TotalTrasladosImpuestoIVA16", Format(TotalTrasladosImpuestoIVA16, "#0.00"))
        End If
        If TotalTrasladosBaseIVA16 > 0 Then
            Totales.SetAttribute("TotalTrasladosBaseIVA16", Format(TotalTrasladosBaseIVA16, "#0.00"))
        End If
        Totales.SetAttribute("MontoTotalPagos", Format(MontoTotalPagos, "#0.00"))
    ...
    Private Sub generacbb()
        ...
        '
        rfcE = GetXmlAttribute(Server.MapPath("~/portalcfd/cfd_storage") & "\s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml", "Rfc", "cfdi:Emisor")
        rfcR = GetXmlAttribute(Server.MapPath("~/portalcfd/cfd_storage") & "\s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml", "Rfc", "cfdi:Receptor")
        total = GetXmlAttribute(Server.MapPath("~/portalcfd/cfd_storage") & "\s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml", "Total", "cfdi:Comprobante")
        UUID = GetXmlAttribute(Server.MapPath("~/portalcfd/cfd_storage") & "\s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml", "UUID", "tfd:TimbreFiscalDigital")
        sello = GetXmlAttribute(Server.MapPath("~/portalcfd/cfd_storage") & "\s7_" & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml", "SelloCFD", "tfd:TimbreFiscalDigital")
    ...
    Private Function CadenaOriginalComplemento() As String
        ...
        Dim FlujoReader As XmlTextReader = Nothing
        Dim i As Integer
        FlujoReader = New XmlTextReader(Server.MapPath("~/portalcfd/cfd_storage/s7_") & serie.Value.ToString & folio.Value.ToString & "_timbrado.xml")
        FlujoReader.WhitespaceHandling = WhitespaceHandling.None
        While FlujoReader.Read()
            Select Case FlujoReader.NodeType
                Case XmlNodeType.Element
