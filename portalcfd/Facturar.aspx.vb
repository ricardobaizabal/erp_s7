...
#Region "Load Initial Values"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            miDataTable.Dispose()
            Session("Columna1") = 0
            If miDataTable.Columns.Contains("uuid") Then
                miDataTable.Columns.Remove("uuid")
                miDataTable.Columns.Add("uuid")
            Else
            ...
            ObjCat.Catalogo(cmbProyecto, "select id, nombre from tblProyecto order by id", 0)
            ...
            If Not String.IsNullOrEmpty(Request("id")) Then
                ...
                proyecto_id = DataControl.RunSQLScalarQuery("select isnull(proyectoid,0) from tblPedidos where cfdid='" & Request("id").ToString & "'")
                DataControl.Catalogo(cmbProyecto, "select id, nombre from tblProyecto order by id", proyecto_id)
                DataControl = Nothing
    ...
    Private Sub CargaCFD()
        ...
        Try
            ...
            If rs.Read() Then
                ...
                ObjData.Catalogo(cmbSucursal, "EXEC pListarSucursales  @clienteid='" & cmbCliente.SelectedValue & "'", rs("sucursalid"))
                ObjData = Nothing
    ...
    Private Sub CargaOrdenCompra()
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("conn").ConnectionString)
        Dim cmd As New SqlCommand("EXEC pPedidos @cmd=18, @cfdid='" & Session("CFD").ToString & "'", conn)
    ...
    Private Sub CargaCliente(ByVal ClienteId As Long)
        ...
        Try
            ...
            If rs.Read() Then
                ...
                Dim ObjData As New DataControl
                ObjData.Catalogo(cmbFormaPago, "select id, id + ' - ' + nombre from tblFormaPago order by nombre", rs("formapagoid"))
                ObjData.Catalogo(cmbUUID, "select top 1000 uuid, convert(varchar(10), fecha_factura, 103) + ' | ' + isnull(serie,'') + convert(varchar(10), folio) + ' - ' + isnull(uuid,'') as folio from tblCFD where isnull(uuid,'')<>'' and (estatus=1 or estatus=3) and serie='F' and clienteid='" & ClienteId.ToString & "' order by fecha_factura desc", 0)
...
#Region "Add Invoice Items"
    ...
    Protected Sub GetCFD()
        ...
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("conn").ConnectionString)
        Dim cmd As New SqlCommand("EXEC pCFD @cmd=1, @clienteid='" & cmbCliente.SelectedValue & "', @sucursalid='" & cmbSucursal.SelectedValue.ToString & "', @monedaid='" & cmbMoneda.SelectedValue.ToString & "', @orden_compra='" & txtOrdenCompra.Text & "', @proyectoid='" & cmbProyecto.SelectedValue.ToString & "', @tipodocumentoid='" & cmbTipoDocumento.SelectedValue.ToString & "'", conn)
    ...
    Protected Sub InsertItem(ByVal id As Integer, ByVal item As GridItem)
        ...
        objdata.RunSQLQuery("EXEC pCFD @cmd=2, @cfdid='" & Session("CFD").ToString & "', @codigo='" & lblCodigo.Text & "', @descripcion='" & txtDescripcion.Text & "', @cantidad='" & txtQuantity.Text & "', @unidad='" & lblUnidad.Text & "', @precio='" & txtUnitaryPrice.Text & "', @inventarioid='" & id.ToString & "', @descuentobit='0', @importe_descuento='" & descuento.ToString & "'")
        objdata = Nothing
        '
    End Sub
    ...
    Protected Sub itemsList_ItemCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles itemsList.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case "cmdView"
                ...
                'ds = ObjData.FillDataSet("EXEC pPedidos @cmd=19, @productoid='" & productoid & "'")
...
#Region "Create Invoice"
    ...
    Protected Sub btnCreateInvoice_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreateInvoice.Click
        If Page.IsValid Then
            ...
            Else
                ...
                m_xmlDOM.Save(Server.MapPath("cfd_storage") & "\" & "s7_" & serie.ToString & folio.ToString & ".xml")
                '
                '   Realiza Timbrado
                '
                If folio > 0 Then
                    Try
                        ...
                        If Timbrado = True Then
                            ...
                            '   Obtiene el UUID
                            '
                            Dim filePath As String = Server.MapPath("~/portalcfd/cfd_storage/s7_") & serie.ToString & folio.ToString & "_timbrado.xml"
                            Dim UUID() As String
                            ReDim UUID(0)
                            '
                            ...
                            '   Genera PDF
                            '
                            If Not File.Exists(Server.MapPath("~/portalcfd/pdf") & "\s7_" & serie.ToString & folio.ToString & ".pdf") Then
                                GuardaPDF(GeneraPDF(Session("CFD")), Server.MapPath("~/portalcfd/pdf") & "\s7_" & serie.ToString & folio.ToString & ".pdf")
                            End If
                        End If
    ...
    Private Function Comprimir()
        ...
        zip.AddFile(Server.MapPath("~/portalcfd/cfd_storage/") & "s7_" & serie.ToString & folio.ToString & ".xml", "")
    ...
    Private Function Descomprimir(ByVal data5 As Byte())
        ...
        If File.Exists(Path & archivo) Then
            System.IO.File.Copy(Path & archivo, Path & "s7_" & serie.ToString & folio.ToString & "_timbrado.xml")
        End If
    ...
    Private Sub SeparaProductos(ByVal cfdid As Long)
        Dim ObjData As New DataControl
        ObjData.RunSQLQuery("exec pSeparaProductos @cmd=1, @cfdid='" & cfdid.ToString & "'")
        ObjData = Nothing
    End Sub

    'Private Sub DescargaInventario(ByVal cfdid As Long)
    '    Dim ObjData As New DataControl
    '    ObjData.RunSQLQuery("exec pDescargaInventario @userid='" & Session("userid").ToString & "', @cfdid='" & cfdid.ToString & "'")
    '    ObjData = Nothing
    'End Sub
    ...
    Private Sub Remisionar()
        ...
        If Not chkAduana.Checked Then
            SQLUpdate = "exec pCFD @cmd=17, @cfdid='" & Session("CFD").ToString & "', @tipodocumentoid='" & cmbTipoDocumento.SelectedValue.ToString & "', @instrucciones='" & instrucciones.Text & "', @aduana='" & nombreaduana.Text & "', @fecha_pedimento='', @numero_pedimento='" & numeropedimento.Text & "', @tipocambio='" & txtTipoCambio.Text & "', @metodopagoid='" & cmbMetodoPago.SelectedValue.ToString & "', @numctapago='" & txtNumCtaPago.Text & "', @condicionesid='" & cmbCondiciones.SelectedValue.ToString & "', @sucursalid='" & cmbSucursal.SelectedValue.ToString & "'"
        Else
            SQLUpdate = "exec pCFD @cmd=17, @cfdid='" & Session("CFD").ToString & "', @tipodocumentoid='" & cmbTipoDocumento.SelectedValue.ToString & "', @instrucciones='" & instrucciones.Text & "', @aduana='" & nombreaduana.Text & "', @fecha_pedimento='" & fechapedimento.SelectedDate.Value.ToShortDateString & "', @numero_pedimento='" & numeropedimento.Text & "', @tipocambio='" & txtTipoCambio.Text & "', @metodopagoid='" & cmbMetodoPago.SelectedValue.ToString & "', @numctapago='" & txtNumCtaPago.Text & "', @sucursalid='" & cmbSucursal.SelectedValue.ToString & "'"
        End If
        ...
        '
        If Not File.Exists(Server.MapPath("~/portalcfd/pdf") & "\s7_" & serie.ToString & folio.ToString & ".pdf") Then
            GuardaPDF(GeneraPDF_Documento(Session("CFD")), Server.MapPath("~/portalcfd/pdf") & "\s7_" & serie.ToString & folio.ToString & ".pdf")
        End If
        '
        '
        '   Descarga Inventario 
    ...
    Private Sub AsignaSerieFolio()
        ...
        If Not chkAduana.Checked Then
            SQLUpdate = "exec pCFD @cmd=17, @cfdid='" & Session("CFD").ToString & "', @tipodocumentoid='" & cmbTipoDocumento.SelectedValue.ToString & "', @instrucciones='" & instrucciones.Text & "', @aduana='" & nombreaduana.Text & "', @fecha_pedimento='', @numero_pedimento='" & numeropedimento.Text & "', @tipocambio='" & txtTipoCambio.Text & "', @metodopagoid='" & cmbMetodoPago.SelectedValue.ToString & "', @formapagoid='" & cmbFormaPago.SelectedValue.ToString & "', @monedaid='" & cmbMoneda.SelectedValue.ToString & "', @numctapago='" & txtNumCtaPago.Text & "', @condicionesid='" & cmbCondiciones.SelectedValue.ToString & "', @sucursalid='" & cmbSucursal.SelectedValue.ToString & "', @proyectoid='" & cmbProyecto.SelectedValue.ToString & "'"
        Else
            SQLUpdate = "exec pCFD @cmd=17, @cfdid='" & Session("CFD").ToString & "', @tipodocumentoid='" & cmbTipoDocumento.SelectedValue.ToString & "', @instrucciones='" & instrucciones.Text & "', @aduana='" & nombreaduana.Text & "', @fecha_pedimento='" & fechapedimento.SelectedDate.Value.ToShortDateString & "', @numero_pedimento='" & numeropedimento.Text & "', @tipocambio='" & txtTipoCambio.Text & "', @metodopagoid='" & cmbMetodoPago.SelectedValue.ToString & "', @formapagoid='" & cmbFormaPago.SelectedValue.ToString & "', @monedaid='" & cmbMoneda.SelectedValue.ToString & "', @numctapago='" & txtNumCtaPago.Text & "', @condicionesid='" & cmbCondiciones.SelectedValue.ToString & "', @sucursalid='" & cmbSucursal.SelectedValue.ToString & "', @proyectoid='" & cmbProyecto.SelectedValue.ToString & "'"
        End If
        ...
        Try
            ...
            While rs.Read
                ...
                If rs("descuento") > 0 Then
                    Concepto.SetAttribute("Descuento", Format(rs("descuento"), "#0.00"))
                End If

                Concepto.SetAttribute("ValorUnitario", Format(rs("precio"), "#0.00"))
                Concepto.SetAttribute("Importe", Format(rs("importe"), "#0.00"))
                ...
                If rs("descuento") > 0 Then
                    Traslado.SetAttribute("Base", Format(rs("importe") - rs("descuento"), "#0.00"))
                Else
                    Traslado.SetAttribute("Base", Format(rs("importe"), "#0.00"))
                End If
    ...
    Private Sub generacbb()
        ...
        '
        rfcE = GetXmlAttribute(Server.MapPath("cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Rfc", "cfdi:Emisor")
        rfcR = GetXmlAttribute(Server.MapPath("cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Rfc", "cfdi:Receptor")
        total = GetXmlAttribute(Server.MapPath("cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Total", "cfdi:Comprobante")
        UUID = GetXmlAttribute(Server.MapPath("cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "UUID", "tfd:TimbreFiscalDigital")
        sello = GetXmlAttribute(Server.MapPath("cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "SelloCFD", "tfd:TimbreFiscalDigital")
    ...
    Private Function GeneraPDF(ByVal cfdid As Long) As Telerik.Reporting.Report
        ...
        Select Case tipoid
            Case 3, 6, 7      ' honorarios y arrendamiento
                ...
                reporte.ReportParameters("txtFechaEmision").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Fecha", "cfdi:Comprobante")
                reporte.ReportParameters("txtFechaCertificacion").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "FechaTimbrado", "tfd:TimbreFiscalDigital")
                reporte.ReportParameters("txtUUID").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "UUID", "tfd:TimbreFiscalDigital")
                reporte.ReportParameters("txtPACCertifico").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "RfcProvCertif", "tfd:TimbreFiscalDigital")
                reporte.ReportParameters("txtSerieEmisor").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "NoCertificado", "cfdi:Comprobante")
                reporte.ReportParameters("txtSerieCertificadoSAT").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "NoCertificadoSAT", "tfd:TimbreFiscalDigital")
                reporte.ReportParameters("txtClienteRazonSocial").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Nombre", "cfdi:Receptor")
                ...
                reporte.ReportParameters("txtClienteRFC").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Rfc", "cfdi:Receptor")
                reporte.ReportParameters("txtSelloDigitalCFDI").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "Sello", "cfdi:Comprobante")
                reporte.ReportParameters("txtSelloDigitalSAT").Value = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "SelloSAT", "tfd:TimbreFiscalDigital")
            ...
            Case Else
                ...
                reporte.ReportParameters("txtEmisorRazonSocial").Value = em_razonsocial
                reporte.ReportParameters("txtLugarExpedicion").Value = expedicionLinea1 & vbCrLf & expedicionLinea2 & vbCrLf & expedicionLinea3
                tipo_comprobante = GetXmlAttribute(Server.MapPath("~/portalCFD/cfd_storage") & "\s7_" & serie.ToString & folio.ToString & "_timbrado.xml", "TipoDeComprobante", "cfdi:Comprobante")
                ...
                If tblRelacionados.Items.Count > 0 Then
                    ...
                    reporte.ReportParameters("txtUUIDRelacionado").Value = "UUID Relacionado: " & uuids
                End If
                If tipoid = 5 Then
...
#End Region
    ...
    Protected Sub serieid_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbTipoDocumento.SelectedIndexChanged
        cmbTipoDocumento.Enabled = False
    End Sub
    ...
    Protected Sub txtSearchItem_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSearchItem.TextChanged
        ...
        'itemsList.Visible = False
        'Dim objdata As New DataControl
        ''gridResults.DataSource = objdata.FillDataSet("exec pCFD @cmd=30, @txtSearch='" & txtSearchItem.Text & "', @clienteid='" & cmbCliente.SelectedValue.ToString & "', @almacenid='" & almacenid.SelectedValue.ToString & "'")
        'Dim cmd = "exec pCFD @cmd=30, @txtSearch='" & txtSearchItem.Text & "', @clienteid='" & cmbCliente.SelectedValue.ToString & "'"
    ...
    '                Dim ObjData As New DataControl
    '                ObjData.RunSQLQuery("exec pPedidos @cmd=20, @cfdid='" & Session("CFD").ToString & "', @partidaid='" & partidaid.Value.ToString & "', @productoid='" & productoId.ToString & "', @cantidad='" & txtCantidad.Text.ToString & "', @inventarioid='" & inventarioId.ToString & "'")
    '                ObjData = Nothing
    ...
    Private Sub ValidaFacturar()
        ...
        Dim ObjData As New DataControl
        partidas_pendientes = ObjData.RunSQLScalarQueryDecimal("exec pPedidos @cmd=21, @cfdid='" & Session("CFD").ToString & "'")
        ObjData = Nothing
    ...
    Sub txtPrecio_TextChanged(sender As Object, e As EventArgs)
        For Each dataItem As Telerik.Web.UI.GridDataItem In itemsList.MasterTableView.Items
            Dim partidaid As String = dataItem.GetDataKeyValue("id").ToString
            Dim txtPrecio As RadNumericTextBox = DirectCast(dataItem.FindControl("txtPrecio"), RadNumericTextBox)
            ...
            Dim ObjData As New DataControl
            ObjData.RunSQLQuery("exec pCFD @cmd=42, @partidaid='" & partidaid.ToString & "', @precio='" & precio.ToString & "'")
            ObjData = Nothing
...
#Region "UUids relacionados"
    ...
    Private Function CfdiRelacionados2() As Boolean
        If (tblRelacionados.Items.Count > 0 And tiporelacionid.SelectedValue < 1) Then
            ...
            Return True
        End If
        If tiporelacionid.SelectedValue > 0 Then
            If tblRelacionados.Items.Count > 0 Then
                Dim Obj As New DataControl
                Dim num As Integer = 0
                If Not String.IsNullOrEmpty(Request("id")) Then
                    num = Obj.RunSQLScalarQuery("select isnull(COUNT(id),0)  from tblUUIDS_Relacionados where cfdid = " & Request("id"))
                Else
                    GetUUIDS_relacionados()
                    num = tblRelacionados.Items.Count
                End If
    ...
    Protected Sub btnAddUiid_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddUiid.Click
        If Not String.IsNullOrEmpty(Request("id")) Then
            Dim ObjData As New DataControl
            ObjData.RunSQLQuery("EXEC pCFD @cmd=45, @uuid_relacionado='" & cmbUUID.SelectedValue.ToString() & "',@cfdid='" & Request("id") & "'")
            ObjData = Nothing
        Else
            ...
            Session("datos") = miDataTable
            Session("uuid") = miDataTable.Rows(0)("UUID").ToString()
    ...
    Protected Sub GetUUIDS_relacionados()
        Dim ObjData As New DataControl
        For i As Integer = 0 To miDataTable.Rows.Count - 1
            mi_variable = miDataTable.Rows(i)("UUID").ToString()
        ...
        Select Case e.CommandName
            Case "cmdDelete"
                ...
                If Not String.IsNullOrEmpty(Request("id")) Then
                    ...
                    ObjData.RunSQLQuery("delete from tblUUIDS_Relacionados where UUID = '" & e.CommandArgument.ToString & "' and cfdid=" & Request("id"))
                    ObjData = Nothing
                Else
                    Dim pAux As New DataTable
                    pAux = miDataTable.Clone()
                    For Each row As DataRow In miDataTable.Rows
                        If row("UUID") <> e.CommandArgument Then
                            pAux.ImportRow(row)
                        End If
                    Next
                    miDataTable = New DataTable
                    miDataTable = pAux.Copy()
                End If
                tblRelacionados_NeedData("on")
        End Select
    End Sub
    ...
    Private Sub tblRelacionados_NeedData(ByVal state As String)
        If Not String.IsNullOrEmpty(Request("id")) Then
            Dim Obj As New DataControl
            tblRelacionados.DataSource = Obj.FillDataSet("select isnull(UUID,'') as UUID from tblUUIDS_Relacionados where cfdid=" & Request("id"))
        Else
            tblRelacionados.DataSource = miDataTable
        End If
