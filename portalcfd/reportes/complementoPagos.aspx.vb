...
#Region "Load Initial Values"
    ...
    Private Sub MuestraLista()
        ...
        cfdlist.DataSource = ObjData.FillDataSet("exec pCFD @cmd=49, @tipodocumentoid='" & tipoid.ToString & "', @clienteid='" & clienteid.SelectedValue.ToString & "', @fhaini='" & fha_ini.SelectedDate.Value.ToShortDateString & "', @fhafin='" & fha_fin.SelectedDate.Value.ToShortDateString & "'")
        cfdlist.DataBind()
        ObjData = Nothing
        '
...
#End Region
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        cacheTable.Columns.Add("rfc", GetType(String))
        cacheTable.Columns.Add("estatus", GetType(String))
        cacheTable.Columns.Add("facturas", GetType(String))
        cacheTable.Columns.Add("fecha_pago", GetType(String))
        ...
        dt = Obj.FillDataSet("exec pCFD @cmd=50, @tipodocumentoid='" & tipoid.ToString & "', @clienteid='" & clienteid.SelectedValue.ToString & "', @fhaini='" & fha_ini.SelectedDate.Value.ToShortDateString & "', @fhafin='" & fha_fin.SelectedDate.Value.ToShortDateString & "'")

        For Each row As DataRow In dt.Tables(0).Rows
            cacheTable.Rows.Add(row("serie"), row("folio"), row("fecha"), row("cliente"), row("rfc"), row("estatus"), row("facturas"), row("fecha_pago"), row("montototal"), row("uuid"))
        Next
        '
        Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture("es-MX")
        Thread.CurrentThread.CurrentUICulture = New CultureInfo("es-MX")
        ...
        ExcelGrid.ExportSettings.FileName = "ReporteComplementoPagos_" & Format(Now(), "ddMMyy HHmmss")
