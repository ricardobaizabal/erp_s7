...
Public Class ordenes_compra
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            filtroFechaInicio.SelectedDate = Now()
            filtroFechaFin.SelectedDate = Now()

            Dim dtCtrl As New DataControl
            dtCtrl.Catalogo(filtroproveedorid, "SELECT id, razonsocial FROM tblMisProveedores order by razonsocial", False)
            dtCtrl.Catalogo(filtrousuarioid, "SELECT id, nombre FROM tblUsuario order by nombre", False)
            dtCtrl = Nothing
    ...
    Private Sub MuestraOrdenes()
        ...
        ordersList.DataSource = ObjData.FillDataSet("exec pOrdenCompra @cmd=1, @proveedorid='" & filtroproveedorid.SelectedValue & "'" & ", @userid='" & filtrousuarioid.SelectedValue & "'" & ", @filtroFechaInicio='" & filtroFechaInicio.SelectedDate & "'" & ", @filtroFechaFin='" & filtroFechaFin.SelectedDate & "'")
        ordersList.DataBind()
        ObjData = Nothing
        '
    ...
    Private Sub DownloadPDF(ByVal ordenId As Long)
        ...
        Dim FilePath = Server.MapPath("~/proveedores/pdf/") & "oc_" & ordenId.ToString & ".pdf"
        ...
        Else
            ...
            Call GuardaPDF(GeneraPDF(ordenId), FilePath)
    ...
    Public Function GeneraPDF(ByVal cfdid As Long) As Telerik.Reporting.Report
        ...
        Dim mensajeria As String = ""
        Dim comentarios As String = ""
        Dim flete_prepagado As String = ""
        Dim proyecto_nombre As String = ""
        Dim proyecto_lugar As String = ""
        Dim usuario_autoriza As String = ""
        ...
        Try
            ...
            If rs.Read() Then
                ...
                mensajeria = rs("mensajeria")
                flete_prepagado = rs("fleteprepagado")
                proyecto_nombre = rs("proyectonombre")
                proyecto_lugar = rs("proyectolugar")
                usuario_autoriza = rs("usuarioautoriza")
            End If
            rs.Close()
        ...
        reporte.ReportParameters("pIdOrdenCompra").Value = clave
        ...
        reporte.ReportParameters("txtMensajeria").Value = mensajeria
        reporte.ReportParameters("txtFletePrepago").Value = flete_prepagado
        reporte.ReportParameters("txtNombreProyecto").Value = proyecto_nombre
        reporte.ReportParameters("txtLugarProyecto").Value = proyecto_lugar
        reporte.ReportParameters("txtComentarios").Value = comentarios
        'reporte.ReportParameters("txtUsuarioSolicita").Value = solicita
        reporte.ReportParameters("txtUsuarioSolicita").Value = solicita
        reporte.ReportParameters("txtUsuarioAutoriza").Value = usuario_autoriza
